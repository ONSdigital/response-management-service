...
BEGIN
   ...
   IF EXISTS (SELECT 1 FROM action.actionplan WHERE actionplanid = p_actionplanid) THEN  
      ...
      IF EXISTS (SELECT 1 FROM action.case WHERE actionplanjobid = p_actionplanjobid) THEN  
         ...
         v_text := '********* JOBID: ' || p_actionplanjobid || ' ACTION PLAN: ' || p_actionplanid || ' COMPLETE ********* (action.actionplan table updated with 
...
lastgoodrundatetime)'; 
         ...
         v_text := '********* JOBID: ' || p_actionplanjobid || ' ACTION PLAN: ' || p_actionplanid || ' COMPLETE ********* (action.actionplanjob table updated with 
